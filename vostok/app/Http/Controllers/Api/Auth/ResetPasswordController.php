...
class ResetPasswordController extends Controller
{
    ...
    public function reset(Request $request)
    {
        ...
        if (DB::table('password_resets')->where('token', '=', $request->token)->where('email', '=', $credential)->doesntExist()) {
            return $this->sendResetFailedResponse($request, 'passwords.token');
        }
        $query = User::where('email', '=', $credential)->orWhere('phone', '=', $credential);
        if ($query->doesntExist()) {
            return $this->sendResetFailedResponse($request, 'passwords.user');
        }

        $this->resetPassword($query->first(), $request->password);

        DB::table('password_resets')->where('email', '=', $credential)->delete();
